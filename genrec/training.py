...
def prefix_examples(sequences, max_history=50, user_ids=None):
    """Standard sequential protocol: every train prefix predicts its next item."""
    if user_ids is not None and len(user_ids) != len(sequences):
        raise ValueError("train_user_ids must be parallel to train_sequences.")
    return [(sequence[max(0, position - max_history):position], sequence[position], user_ids[row])
            if user_ids is not None else (sequence[max(0, position - max_history):position], sequence[position])
            for row, sequence in enumerate(sequences) for position in range(1, len(sequence))]


def make_batches(examples, batch_size, device):
    for start in range(0, len(examples), batch_size):
        part = examples[start:start + batch_size]; width = max(len(example[0]) for example in part)
        history = torch.zeros(len(part), width, dtype=torch.long)
        for row, example in enumerate(part):
            values = example[0]; history[row, -len(values):] = torch.tensor(values)
        batch = {"history": history.to(device), "target": torch.tensor([example[1] for example in part], device=device)}
        if len(part[0]) == 3: batch["user"] = torch.tensor([example[2] for example in part], device=device)
        yield batch
...
class Trainer:
    def __init__(self, epochs=None, steps=None, batch_size=256, lr=.01, optimizer="adagrad", warmup_steps=10_000,
                 ...
                 early_stop_metric="NDCG@10", min_delta=0.0, checkpoint_path=None, eval_every_epochs=None):
        if steps is None and epochs is None: epochs = 5
        if steps is not None and epochs is not None: raise ValueError("Choose either steps or epochs, not both.")
        if eval_every_steps and eval_every_epochs: raise ValueError("Choose either step-level or epoch-level validation, not both.")
        ...
        self.eval_every_steps, self.eval_every_epochs, self.early_stop_patience = eval_every_steps, eval_every_epochs, early_stop_patience
    ...
    def fit(self, model, dataset, validation_examples=None):
        ...
        examples = prefix_examples(dataset.train_sequences, self.max_history, getattr(dataset, "train_user_ids", None))
        history = []
        ...
        self._log("train_started", epochs=self.epochs, steps=target_steps, batch_size=self.batch_size, lr=self.lr, optimizer=self.optimizer_name,
                  warmup_steps=self.warmup_steps, device=str(self.device), train_examples=len(examples), eval_batch_size=self.eval_batch_size, eval_every_steps=self.eval_every_steps, eval_every_epochs=self.eval_every_epochs,
        ...
        while target_steps is None or step < target_steps:
            ...
            if validation_examples and self.eval_every_epochs and epoch % self.eval_every_epochs == 0:
                metrics = self.evaluate(model, validation_examples); value = metrics[self.early_stop_metric]
                improved = value > best_metric + self.min_delta
                if improved:
                    best_metric, best_step, stale = value, step, 0
                ...
                self._log("validation_completed", step=step, epoch=epoch, metrics=metrics, monitored_metric=self.early_stop_metric,
                          monitored_value=value, improved=improved, best_metric=best_metric, best_step=best_step, stale_evaluations=stale)
                if self.early_stop_patience is not None and stale >= self.early_stop_patience:
                    stopped_early = True; break
