...
`rqvae` now defaults to the TIGER topology and optimization protocol: latent size 32, hidden layers `512,256,128`, three 256-way codebooks, β=0.25, Adagrad(0.4), batch size 1024, and 20,000 epochs. Its Adagrad accumulator starts at 0.1, matching common TensorFlow-style Adagrad stability rather than PyTorch's zero-accumulator first jump. K-means codebook initialization is enabled by default and diagnostics record reconstruction loss, residual error, per-level codebook use, collisions, and a fingerprint of the exact input representation. Use `--hidden-dims`, `--latent-dim`, `--optimizer`, `--lr`, `--epochs`, `--no-kmeans-init`, and `--quantization-warmup-steps` for named ablations; use a distinct index `--artifact-name` for each one.
